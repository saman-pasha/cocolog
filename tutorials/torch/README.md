...
| 31-tensor-expressions | tutorial 30 again, in the syntax the predicates were asking for: `loss(X, Y, W, B) ::= mean((X matmul W + B - Y) ^ 2.0)` a defined function, the step a PROCEDURE -- a DCG rule of bindings `V = E`, run by `proc/1`, which frees what it made -- and `expr//2` behind it all, the same list of goals under both paths; on six rows |
