...
main :-
    ...
    format("~n-- HTTPS IS ONE OPTION, and only the transport changes~n"),
    format("~n"),
    format("     httpd_serve(9443, [ tls([ certificate('node.crt'),~n"),
    format("                               key('node.key'),~n"),
    format("                               authority('ca.crt') ]),~n"),
    format("                         workers(4) ]).~n"),
    format("~n"),
    format("   A connection became a TAGGED TERM -- plain(S) or~n"),
    format("   secure(S) -- and five predicates dispatch on it. Routing,~n"),
    format("   keep-alive, the path rules and httpd_answer/3 are the~n"),
    format("   same code on both, so HTTPS cannot drift away from HTTP~n"),
    format("   by being maintained separately.~n"),
    format("~n"),
    format("   AND A PAGE KNOWS WHO IS ON THE CONNECTION. The handshake~n"),
    format("   settled it against the authority before a byte moved, and~n"),
    format("   it arrives as two headers a page reads like any other:~n"),
    format("~n"),
    format("     httpd_page('/ledger', Req, reply(200, [], done)) :-~n"),
    format("         http_header(Req, 'Tls-Peer-Permissions', G),~n"),
    format("         atomic_list_concat(Gs, ',', G),~n"),
    format("         member(One, Gs), ca_covers(One, 'ledger.write').~n"),
    format("~n"),
    format("   THE CLIENT'S OWN Tls-Peer-* HEADERS ARE STRIPPED FIRST,~n"),
    format("   on both transports. A client may send any header it~n"),
    format("   likes, and a server that merely ADDED its own would~n"),
    format("   leave two -- with the client's first, which is the one~n"),
    format("   http_header/3 finds. That is the standard reverse-proxy~n"),
    format("   hole. On a PLAIN connection they are stripped and not~n"),
    format("   replaced, so a page that trusts them is closed to port 80~n"),
    format("   by construction.~n~n"),
        format("done~n").
