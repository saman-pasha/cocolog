...
main :-
    ...
    format("~n-- a lock is for what the threads DO share~n"),
    format("   Nothing above needed one: two machines share no cell, and a~n"),
    format("   channel carries its own lock. What threads DO share is~n"),
    format("   everything outside the interpreter -- a file two of them~n"),
    format("   append to, a socket handle, this terminal -- and taking~n"),
    format("   turns at that is what a mutex is for.~n"),
    mutex_create(Lock),
    with_mutex(Lock, Answer is 6 * 7),
    must('with_mutex proves the goal under the lock', Answer, 42),
    catch(with_mutex(Lock, throw(oops)), Ball, true),
    must('a throw comes back out of it unchanged', Ball, oops),
    channel_new(Ask),
    thread_create(( mutex_trylock(Lock)
                  -> mutex_unlock(Lock), channel_send(Ask, free)
                  ;  channel_send(Ask, still_held) ), AskId),
    channel_recv(Ask, 5000, State),
    thread_join(AskId, _),
    must('and the lock was released ANYWAY, which is the point', State, free),
    format("   ASKED FROM ANOTHER THREAD ON PURPOSE. These mutexes are~n"),
    format("   RECURSIVE, as SWI's are, so that with_mutex can nest -- and~n"),
    format("   a recursive mutex says yes to a trylock from the thread~n"),
    format("   that already holds it. Asking here would have answered~n"),
    format("   `free' whether it was or not.~n"),
    format("   `with_mutex/2' is the form to use for exactly this reason:~n"),
    format("   a bare mutex_lock with a goal that raises after it holds~n"),
    format("   the lock for the life of the process.~n"),

    format("~n-- and an ATOM is a lock the whole process shares~n"),
    with_mutex(page_log, Named = ok),
    must('no handle passed to anybody', Named, ok),
    format("   Made on first use, shared by every thread, never collected.~n"),
    format("   It is the only way a thread that was handed nothing can~n"),
    format("   share a lock -- an httpd worker proves its page on a~n"),
    format("   machine with no term of yours in it, so `with_mutex(log,~n"),
    format("   Goal)' in each worker is one lock and a handle is not.~n"),

    format("~n-- a condition variable says WAKE UP, to everybody at once~n"),
    cond_create(Gate), mutex_create(GateLock),
    channel_new(Ready), channel_new(Woke),
    thread_pool(3, ( mutex_lock(GateLock),
                     channel_send(Ready, r),
                     cond_wait(Gate, GateLock),
                     mutex_unlock(GateLock),
                     channel_send(Woke, up) ), Waiters),
    channel_recv(Ready, 5000, _), channel_recv(Ready, 5000, _), channel_recv(Ready, 5000, _),
    mutex_lock(GateLock), cond_broadcast(Gate), mutex_unlock(GateLock),
    channel_recv(Woke, 5000, W1), channel_recv(Woke, 5000, W2), channel_recv(Woke, 5000, W3),
    thread_join_all(Waiters),
    must('one broadcast, three threads awake', [W1, W2, W3], [up, up, up]),
    format("   A CHANNEL COULD NOT HAVE SAID THAT. One message goes to one~n"),
    format("   receiver, so a start gate over a channel means sending N~n"),
    format("   messages to a producer that has to know N. A broadcast does~n"),
    format("   not know or care how many are listening.~n"),
    format("   EACH WAITER SENDS `ready' WHILE HOLDING THE MUTEX, which is~n"),
    format("   not decoration: it is what stops the broadcast arriving~n"),
    format("   before the last thread is inside cond_wait, since the~n"),
    format("   signaller cannot take the mutex until every waiter has~n"),
    format("   released it -- and the wait is where they release it.~n"),
    format("   AND A BARE cond_wait/2 RETURNS SPURIOUSLY: POSIX allows it,~n"),
    format("   so a wait outside a loop testing what you are waiting FOR~n"),
    format("   is a bug that reproduces once a week. cond_wait_until/3 is~n"),
    format("   that loop, with the goal tested under the mutex.~n"),
