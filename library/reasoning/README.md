...
| `translate.pl` | a language lesson as a knowledge base and a translation as a proof over it: `reason_translate/2,3` takes a simple sentence -- singular or plural, denied or not, a statement or a question -- between English and the language the lesson teaches, asking the knowledge base what a word means, what it is, its gender, its plural (stated, or by an ending rule), the word for `not` and whether an adjective follows its noun -- and knowing no word of the language itself; `reason_untranslated/2` names the words a lesson left out |
