...
| `translate.pl` | a language lesson as a knowledge base and a translation as a proof over it: `reason_translate/2,3` takes a simple sentence -- a subject, a verb and what follows it; singular or plural, in any person, denied or not, present, past, future or perfect, with a pronoun, a possessive, a number, a prepositional phrase, an adverb or two phrases joined; a statement or a question (yes or no, what, who, where, when, which) -- between English and the language a lesson teaches, asking the knowledge base what a word means, what it is, its gender, its plural, its past, its future, its participle and its persons (stated, or by an ending rule), the word for `not`, whether an adjective follows its noun and whether a pronoun precedes the verb -- and knowing no word of the language itself; `reason_learn/3` learns a lesson under its own name so that several languages sit in one knowledge base, and `reason_untranslated/2` names the words a lesson left out |
