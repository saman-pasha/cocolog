...
| file | what |
|---|---|
| `reason.pl` | a paragraph of controlled English in, predicates out: facts, `neg/1` facts, rules, and a question as a GOAL with a variable where `who`, `what` or `where` stood; `truth/2`, four-valued; `reason_ask/2`, a question answered with the fact, the rule or the denial it rests on; `reason_refused/2`, which sentence would not parse |
