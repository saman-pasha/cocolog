...
| `normalise.pl` | the training data for the network: the grammar's twenty-seven shapes as a generator with gold tags, nine noise transforms that carry the tags, and the assembler the round trip holds them to -- over the lexicon files in `lexicon/`, read as needed and never written into the code |
...
twenty-seven shapes and nine transforms now -- the last four a place after
an object, `rents a flat in Bristol', which a noise transform had taught the
network to DROP until the grammar learned to read it as rent_in/3. The
