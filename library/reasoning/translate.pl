...
tr_read_ir(Piece0, Stop0, From, S, Stop) :-
    ...
    tr_tail_mark(Side, Words0t, Stop1, Words0, Stop),
...
tr_part_words(Codes, Ws) :-
    (   ( memberchk(0'-, Codes) ; tr_inner_capital(Codes) ; tr_two_capitals(Codes), tr_has_small(Codes) ),
...
tr_aside_out(To, Kind, Inner, Noun, Number, Outs) :-
    ...
    ->  (   To == english
        ...
        tr_comps_out(To, Cs, Noun, Number, _, CO, CA), append([[o(PP, lower)], CO, CA], IO)
...
tr_read0(Side, statement, Words0, S) :-
    ...
    ;   \+ tr_names_parted(Side, Rest0), \+ tr_name_opens_clause(Side, Rest0),
        ...
        tr_denials_in_front(Side, Rest, Advs0, Advs1), tr_advs_join(Side, Rest, C1, Advs1, C2),
        S0 = s(A1, Su1, G1, C2)
    ),
    S0 = s(A, Su, G, Comps0),
...
tr_read_statement(Side, Words, Asked, S) :-
    ...
    ->  Key = rs(Side, Words, Asked), tr_failed_set(F0),
        (   get_assoc(Key, F0, _) -> fail
        ;   tr_read_statement0(Side, Words, Asked, S0) -> tr_duty_fix(S0, S1), tr_vq_add(Side, Words, S1, S)
        ;   tr_failed_set(F1), put_assoc(Key, F1, x, F2), nb_setval('$tr_failed', F2), fail
        )
    ;   tr_read_statement0(Side, Words, Asked, S0), tr_duty_fix(S0, S1), tr_vq_add(Side, Words, S1, S)
    ).
...
tr_read_generic(Side, Words0, Asked, S) :-
    ...
    (   Side == foreign, Subject2 = null(_, _), tr_solve(intransitive(L)), memberchk(obj(_), Comps0)
    ->  !, fail
    ;   true
    ),
...
tr_subject_after(Words, FP, FN, Rest, Subject) :-
    ...
    forall(member(X, PPs), ( X = pp(P, _), tr_means_of(foreign, P) ; X = appos(_) ; X = cnj(_) )),
    \+ last(PPs, cnj(_)),
...
%% plural follows it.
tr_number_agrees(foreign, with(NP, [pp(P, NP2)|_]), plural) :-
...
tr_share_word(W) :- tr_lexeme(foreign, W, L, _), tr_solve(mean(L, E)), memberchk(E, [majority, minority]), !.
...
tr_subject(foreign, _, [w(V, _)|Ws], third, singular, infs(F, Cs)) :-
...
tr_np(Side, [D|Ws0], np(det(DK, DL, D), none, AdjsM, named(G, Ws), Number)) :-
    ...
    RevPre = [LastPre|_], ( LastPre = deg(_, LA) -> true ; LA = LastPre ),
    ...
    forall(member(A, Pre), ( A \= w(_, _) -> tr_adj_word(Side, A) ; \+ tr_coord(Side, A), tr_adj_word(Side, A), tr_is(Side, A, adjective) )),
    findall(B, ( member(A, Pre), ( Side == foreign, A = w(_, _) -> B = pre(A) ; B = A ) ), AdjsM),
    tr_det_gender(Side, D, DL, G, Number), !.
...
tr_phrase_aside(Side, Ws, NP, Rest) :-
    append(PW, [comma, w(P, PC)|R1], Ws), PW \== [], \+ memberchk(comma, PW),
    ...
    tr_phrase_np(Side, PW, NP1), NP1 = np(_, _, _, _, _),
    tr_rel_agreement(Side, P, NP1, Cs0, Cs1), tr_rel_case(PC, Cs1, Cs),
...
tr_write(To, Kind, s(Asked, Subject, g(L, T, A, Neg0), Comps00), Outs) :-
    %% the quotation that opened on the verb (tr_vq_add/4)
    ( selectchk(vq, Comps00, Comps) -> VQ = yes ; Comps = Comps00, VQ = no ),
    ...
    ->  en_group(LT, Person, Number, T, A, Neg, Statement0, Front, Tail),
        tr_vq_mark(VQ, Statement0, Statement),
    ...
    ;   %% the impersonal word goes AFTER the object pronouns where the
        ...
        fo_group(LT, Person, Number, T, A, Neg, Clitics1, Group0),
        tr_vq_mark(VQ, Group0, Group),
        append(CompOuts, Advs, Rest),
...
tr_np_out(english, rel(np(Det, Num, Adjs, w(NW, NC), Nb), L, Comps0), Outs, Noun, Number) :-
    \+ ( member(C, Comps0), C \= agr(_) ),
    tr_lexeme_across(L, english, LT), en_participle_of(LT, PP),
    append(Adjs, [ptc(PP)], Adjs1),
...
tr_np_out(To, rel(NP, L, Comps0), Outs, Noun, Number) :- !,
    ...
    ( select(agr(GA), Comps0, Comps1) -> true ; Comps1 = Comps0, GA = none ),
    ...
    append([NPOut, Deg, [o(PP, PC)|CompOuts], Close], Outs).
...
tr_pinf_clitic_out(dat(w(W, _)), T) :- tr_solve(mean(T, W)), tr_solve(dative(T)), !.
