...
bd_header([ '%% blocklist.pl -- GENERATED by tools/coco-agent/build.pl. Do not edit.',
            '%%',
...
bd_inputs(Files) :-
    ...
    bd_path('tools/coco-agent/build.pl', E),
    bd_path('tools/coco-agent/clauses.pl', F),
    append(A, B, AB), append(AB, C, ABC), append(ABC, D, ABCD),
    append(ABCD, [E, F], Files).

bd_outputs([J, P]) :-
    bd_path('tools/coco-agent/blocklist.json', J),
    bd_path('tools/coco-agent/blocklist.pl', P).
