...
%% ======================================================================
%% ---- cc_batch: many documents, one process ---------------------------
%% ======================================================================

%% cc_batch is det.
%% Read a length-prefixed stream of documents from $COCO_CC_BATCH and answer
%% every clause of each.
%%
%% ONE PROCESS FOR THE WHOLE WORKLOAD, which is the only thing that makes a
%% cocolog reader affordable to the Python tools. build.py asks for 525 small
%% documents -- mostly Cicili string literals out of a *X-prolog* table -- and
%% a process apiece at 6ms is 3.7 seconds against the 0.19 it takes today.
%% Batched, it is one start-up and one walk.
%%
%% THE FRAMING IS A BYTE COUNT, NOT A DELIMITER, because a document is
%% arbitrary source: any separator would eventually appear inside one, and the
%% failure would be a silently truncated clause list rather than an error.
%%
%%     <decimal byte count> NEWLINE <exactly that many bytes>   repeated
%%
%% The answer is one `=<index>' line per document followed by that document's
%% clause rows in the same tab-separated shape cc_dump uses, so both share a
%% reader on the other side.
cc_batch :-
    getenv('COCO_CC_BATCH', File),
    read_file_to_codes(File, Codes),
    cc_batch_docs(Codes, 0).

cc_batch_docs([], _) :- !.
cc_batch_docs(Codes, I) :-
    cc_batch_len(Codes, N, Rest),
    !,
    cc_take(N, Rest, Doc),
    cc_drop(N, Rest, Tail),
    format("=~w~n", [I]),
    cc_batch_one(Doc),
    I1 is I + 1,
    cc_batch_docs(Tail, I1).
%% A TRAILING FRAGMENT THAT IS NOT A FRAME IS AN ERROR, not something to skip:
%% a truncated batch file means the caller and this disagree about the framing,
%% and answering the documents that did parse would hide that.
cc_batch_docs(_, I) :-
    throw(error(coco_batch_error(bad_frame, I), cc_batch/0)).

cc_batch_len(Codes, N, Rest) :-
    cc_digits(Codes, [D|Ds], [10|Rest]),
    number_codes(N, [D|Ds]).

cc_digits([C|T], [C|Ds], Rest) :- C >= 0'0, C =< 0'9, !, cc_digits(T, Ds, Rest).
cc_digits(Rest, [], Rest).

cc_batch_one(Doc) :-
    cc_split_clauses(Doc, Spans),
    cc_slices(Spans, 0, Doc, Slices),
    cc_heads_from(Spans, Slices, Clauses),
    forall(member(C, Clauses), cc_batch_row(C)).

cc_batch_row(cc_clause(at(O, L, Col, Len), Head, Kind)) :-
    cc_dump_head(Head, Name, Arity),
    cc_dump_kind(Kind, KA),
    format("~w\t~w\t~w\t~w\t~w\t~w\t~w~n", [O, L, Col, Len, Name, Arity, KA]).
