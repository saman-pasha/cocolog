...
cd_traps(Path) :-
    ...
    ->  Path = P
    ;   cd_path('tools/cocolint/traps.jsonl', Path)
    ).
...
cd_facts_header(
  [ '%% traps.pl -- GENERATED by tools/cocolint/card.pl. Do not edit.',
    '%%',
...
cd_facts_path(P) :- cd_path('tools/cocolint/traps.pl', P).
...
cd_facts_stale :-
    ...
    ->  true
    ;   cd_traps(T),
        cd_path('tools/cocolint/card.pl', Self),
        time_file(F, TF),
        member(In, [T, Self]),
        exists_file(In),
        time_file(In, TI),
