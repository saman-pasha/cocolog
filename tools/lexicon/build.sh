#!/bin/sh
# Writes library/reasoning/lexicon/{noun,class,adj,vt,vi,vpp,adverb,place,unit,language,prose,known_*}.txt
# from WordNet 3.0, with the cocolog beside this script -- library/reasoning/lexicon/build.pl
...
COCOLOG_LIBRARY="$ROOT/library${COCOLOG_LIBRARY:+:$COCOLOG_LIBRARY}" \
  exec ./cocolog -s library/reasoning/lexicon/build.pl -- --wordnet "${WORDNET:-/usr/share/wordnet}" "$@"
