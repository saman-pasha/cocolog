...
for c in files trace vacuum repl tunnel tensors library bigint zigurat-lib tcp engine thread http curl ray serialize httpd httpd-tls crypto tls zigurat-tls tutorials colab groups ruler; do
  [ -n "$1" ] && [ "$1" != "$c" ] && continue
  printf '%-10s ' "$c"
