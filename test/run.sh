...
for c in files trace vacuum repl script tunnel tensors torch-graph torch-grad torch-replay library bigint zigurat-lib tcp engine meter thread process text os kbs http curl ray hex astar serialize httpd httpd-tls crypto tls zigurat-tls tutorials colab lint argv string directives groups ruler; do
  [ -n "$1" ] && [ "$1" != "$c" ] && continue
  printf '%-10s ' "$c"
