#!/bin/sh
# Builds library(aes) -- Zigurat's AES as cocolog predicates, as a LOADABLE
# module. Leaves aes.so on the library path.
#
...
# for a cipher most programs never call. A cocolog built where
...
# aes.cicili carries none -- the trick modules/bigint/build.sh uses.
...
if [ ! -f "$ZIGURATIP/home/include/aes.hpp" ]; then
  echo "aes: no aes.hpp in $ZIGURATIP/home/include -- build ZiguratIP first" >&2
  exit 1
fi
