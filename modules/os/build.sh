#!/bin/sh
# Builds library(os) -- which system, who am I, cores, environment, tools -- as a
# LOADABLE module.
#
...
mkdir -p "$OUT"
( cd "$CICILI" && sbcl --script cicili.lisp --release "$HERE/os.cicili" )
...
"$CC" -shared -fPIC -O3 -o "$OUT/os.so" "$HERE/os.c"
echo "built $OUT/os.so"
