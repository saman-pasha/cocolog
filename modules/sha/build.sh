#!/bin/sh
# Builds library(sha) -- Zigurat's SHA family and HMAC as cocolog predicates, as a LOADABLE
# module. Leaves sha.so on the library path.
#
...
# for a hash most programs never call. A cocolog built where
...
# sha.cicili carries none -- the trick modules/bigint/build.sh uses.
...
if [ ! -f "$ZIGURATIP/home/include/shahelper.hpp" ]; then
  echo "sha: no shahelper.hpp in $ZIGURATIP/home/include -- build ZiguratIP first" >&2
  exit 1
fi
