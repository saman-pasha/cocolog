#!/bin/sh
# Builds library(tls) -- a secure connection over ZiguratIP's TLS, as cocolog
# predicates, as a LOADABLE
# module. Leaves tls.so on the library path.
#
...
# TLS most programs never call. A cocolog built where
...
# tls.cicili carries none -- the trick modules/bigint/build.sh uses.
...
if [ ! -f "$ZIGURATIP/home/include/tlsstream.hpp" ]; then
  echo "tls: no tlsstream.hpp in $ZIGURATIP/home/include -- build ZiguratIP first" >&2
  exit 1
fi
...
( cd "$CICILI" && sbcl --script cicili.lisp --release "$HERE/tls.cicili" )

"$CXX" -shared -fPIC -O3 -std=c++17 \
    -Wno-parentheses-equality -Wno-dangling-else \
    -I"$HERE/zigheaders" \
    -o "$OUT/tls.so" "$HERE/coco-tls.cpp" \
    -L"$ZIGURATIP/home/lib" \
    -lSocketIO -lCryptography -lCore -lStreamIO -lEncoding -lConfiguration \
    -lThreading -lCompression -lssl -lcrypto -lpthread \
    -Wl,-rpath,"$ZIGURATIP/home/lib"
echo "built $OUT/tls.so"
